...
namespace WebAPI_RPG.Services.CharacterService
{
    public class CharacterService : ICharacterService
    {
        ...
        public async Task<ServiceWrapper<List<GetCharacterDTO>>> DeleteCharacterService(int id)
        {
            ...
            try
            {
                Character character = await _context.Characters.Where(u => u.User.id == GetUserId()).FirstAsync(c => c.Id == id);
                _context.Characters.Remove(character);
                await _context.SaveChangesAsync();
                List<Character> characters = await _context.Characters.Where(u => u.User.id == GetUserId()).ToListAsync();
                _wrapper.Data = characters.Select(c => _mapper.Map<GetCharacterDTO>(c)).ToList();
            }
            catch (Exception ex)
            {
